 ...
 * 					 HumanPlayer.java, RandomOlayer.java, AlphaBetaPlayer.java, GreedyPlayer.java, MinimaxPlayer.java
 ...
 * Written:       	 4/29/2022
 * 
 * Credits:        	Apoorva Kanekal, Github, Recitation, Kenzie Braun, StackExchange, Java Textbook, Geeks for Geeks, Alpha Beta Pruning Video
 **************************************************************************** */
/**
 * sets up alpha beta pruning and an alpha beta player to make my algorithm more efficient and effective
 * 
 * 
...
public class AlphaBetaPlayer {
		...
		public String name() {
			return "AlphaBeta"; //names alpha beta player
		}
		...
		private int alphabeta(GameTree node, int depth, boolean maxminizingPlayer, int alpha, int beta, Arbitrator arb) {
			...
			if(maxminizingPlayer) {
				...
				for(GameTree child: node.children) {
					...
					//actual alpha beta pruning implemented
					if(alpha >= beta) {
						break;
					}
				}
...
//creates nodes in the GameTree and sets up instance variables, provides setup for the game and player
